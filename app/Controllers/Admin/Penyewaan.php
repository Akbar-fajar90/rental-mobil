...
class Penyewaan extends BaseController
{
    ...
    public function validateDocuments(int|string $id_pelanggan)
    {
        $id_pelanggan = (int)$id_pelanggan;
        /** @var array|object|null $pelanggan */
    ...
    public function checkAvailability(int|string $id_mobil, string $tgl_sewa, string $tgl_kembali)
    {
    ...
    public function approve(int|string $id)
    {
        ...
        // --- RE-CHECK AVAILABILITY START ---
        // Cek apakah mobil sudah disewa di tanggal yang sama oleh penyewaan lain yang SUDAH DISETUJUI
        $existing = $this->penyewaanModel
            ->where('id_mobil', $sewa->id_mobil)
            ->where('id_sewa !=', $id)
            ->where('status_pengajuan', 'disetujui')
            ->groupStart()
                ->where('tgl_sewa <=', $sewa->tgl_kembali_rencana)
                ->where('tgl_kembali_rencana >=', $sewa->tgl_sewa)
            ->groupEnd()
            ->first();
        
        if ($existing) {
            session()->setFlashdata('error', 'Mobil sudah disetujui untuk penyewaan lain pada periode yang sama!');
            return redirect()->to('/admin/penyewaan');
        }
        // --- RE-CHECK AVAILABILITY END ---
    ...
    public function verifikasiDokumen(int|string $id)
    {
        $id = (int)$id;
        /** @var array|object|null $sewa */
        $sewa = $this->penyewaanModel->select('t_sewa.*, t_pelanggan.nama as nama_pelanggan, t_pelanggan.nik, t_pelanggan.no_sim, t_pelanggan.foto_identitas')
                    ->join('t_pelanggan', 't_pelanggan.id_pelanggan = t_sewa.id_pelanggan')
                    ->find($id);
        ...
        return view('admin/penyewaan_verifikasi', $data);
    }
    
    // ========== SUBMIT VERIFIKASI DOKUMEN ==========
    public function submitVerifikasi(int|string $id)
    {
        $id = (int)$id;
        $status = $this->request->getPost('status_dokumen'); // 'valid' atau 'tidak valid'
        $catatan = $this->request->getPost('catatan_dokumen');
        
        if (!$status) {
            return redirect()->back()->with('error', 'Silakan pilih status verifikasi!');
        }
        ...
        $msg = ($status == 'valid') ? 'Dokumen diverifikasi valid!' : 'Dokumen ditandai tidak valid!';
    ...
    // ========== DOWNLOAD DOKUMEN ==========
    public function downloadDokumen(string $jenis, int|string $id)
    {
        $id = (int)$id;
        /** @var array|object|null $sewa */
        $sewa = $this->penyewaanModel->select('t_sewa.*, t_pelanggan.foto_identitas')
                    ->join('t_pelanggan', 't_pelanggan.id_pelanggan = t_sewa.id_pelanggan')
                    ->find($id);
        ...
        if ($jenis == 'ktp') {
            // Cek di t_sewa dulu (snapshot), jika kosong cek di profil pelanggan
            $filename = $sewa->dokumen_ktp ?: $sewa->foto_identitas;
